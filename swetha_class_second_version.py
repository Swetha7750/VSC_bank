            ...
            if self.wanted_body in self.body_index:
                ...
                if wanted_index < len(bodies):
                    ...
                    if not (math.isnan(position.x) or math.isnan(position.y) or math.isnan(position.z)):
                        ...
                        if rotation:
                            self.rot_matrix = np.array(rotation).reshape(3, 3)
...
class PIDController:
    ...
    def control_loop(self, database=None):
        ...
        while time.time() - start_time < 300:  # 5 minutes
            ...
            if database:
                database.append_values(elapsed_time)
            ...
            logger.info(
                ...
                f"Command: X_cmd={control_x:.3f}, Y_cmd={control_y:.3f} | "
...
class mav_work:
    ...
    def rc_override(self):
        ...
        rc_channel_values[1] =  self.pid_controller.pwm_x
        rc_channel_values[2] = self.pid_controller.pwm_y
...
class data_base:
    ...
    def append_values(self, elapsed_time):
        ...
        current_time = time.time()
        dt = current_time - self.pid_controller.last_time if not self.pid_controller.first_call else 0.1
        
        # Store the current PID outputs that were calculated in the control loop
        control_x = self.pid_controller.kp * error_x + self.pid_controller.ki * self.pid_controller.integral_x + \
                   self.pid_controller.kd * (error_x - self.pid_controller.last_error_x) / dt if dt > 0 else 0
        control_y = self.pid_controller.kp * error_y + self.pid_controller.ki * self.pid_controller.integral_y + \
                   self.pid_controller.kd * (error_y - self.pid_controller.last_error_y) / dt if dt > 0 else 0
    ...
    def save_data(self):
        ...
        filename = f'trajectory_data_{int(time.time())}.csv'
        try:
