...
class PIDController:
    def __init__(self, qtm_tracker):
        ...
        # PID parameters
        self.kp = 0.8
        self.ki = 0.0 #previously it was 0.1
        self.kd = 0.0
        ...
        # Actuator limits
        self.operational_limit = 0.0872665  # in radians [5° degrees]
        self.min_angle = -0.523599
        self.max_angle = 0.523599 #30 degrees
        self.min_pwm = 1000
        self.max_pwm = 2000
        ...
        self.control_x, self.control_y =0,0

    def theta_global_to_body_frame(self,theta,phi, psi): # the function that does the global to body transformation
        self.pitch_body = ((math.cos(psi)*theta)+(math.sin(psi)*phi))

        return self.pitch_body
        
    def phi_global_to_body_frame(self,theta,phi, psi):
        self.phi_body = ((-(math.sin(psi)*theta))+((math.cos(psi)*phi)))
        return self.phi_body
    ...
    def angle_to_pwm(self, cmd_angle):
        ...
        #limiting_angle=max(min(cmd_angle,self.max_angle ),self.min_angle)
        
        limited_angle = max(min(cmd_angle, self.operational_limit), -self.operational_limit)
        #truncated_value = math.trunc(limited_angle * 1000) / 1000
        
        # Convert to PWM: 1500 ± 500 * (angle / max_angle)
        rc_pwm = int(round(1500 + limited_angle/0.523599 * 500, 0))
        #rc_pwm = np.round(1500 + (limited_angle / 0.523599) * 500).astype(int)
    ...
    def control_loop(self, database=None):
        ...
        while time.time() - start_time < 300:  # 5 minutes
            ...
            # Capture phase
            if not self.captured and elapsed_time <= 30:
                # self.capture_x = self.measured_x_body       #position['x']
                # self.capture_y = self.measured_y_body       #position['y']
                # self.set_target(self.capture_x, self.capture_y)
                self.set_target(0.0, 0.0)
                self.captured = True
                logger.info(f"Position captured at t={elapsed_time:.1f}s: X={self.capture_x:.3f}, Y={self.capture_y:.3f}")
            ...
            self.control_x, self.control_y = self.compute_pid(error_x, error_y, dt)
            self.theta_body = self.theta_global_to_body_frame(self.control_x, self.control_y, yaw_rad)
            self.phi_body = self.phi_global_to_body_frame(self.control_x, self.control_y, yaw_rad)
            self.theta_body = -max(min(self.theta_body, self.output_limits[1]), self.output_limits[0])
             
            self.phi_body = max(min(self.phi_body, self.output_limits[1]), self.output_limits[0])
            # self.pwm_x = int(round(1500 + control_x/0.523599 * 500, 0))
            # self.pwm_y = int(round(1500 + control_y/0.523599 * 500, 0))
            #print(math.trunc(self.control_x * 1000) / 1000)
            
            # self.pwm_x = self.angle_to_pwm(math.trunc(self.control_x * 1000) / 1000)
            # self.pwm_y =self.angle_to_pwm(math.trunc(self.control_y * 1000) / 1000 )
            self.pwm_x = self.angle_to_pwm(self.theta_body)
            self.pwm_y = self.angle_to_pwm(self.phi_body)

            #print(f"control_x: {control_x} and pwm_x: {self.pwm_x}\n")
            # Store data if database is provided
            if database:
                 database.append_values(elapsed_time, position, error_x, error_y, self.control_x, self.control_y)
            ...
            logger.info(
                ...
                #f"Command: theta_command={math.trunc(self.control_x * 1000) / 1000}, pitch_cmd={math.trunc(self.control_y * 1000) / 1000} | "
                f"Command: theta_command={self.theta_body:.3f}, pitch_cmd={self.phi_body:.3f} | "
...
class mav_work:
    ...
    def rc_override(self):
        ...
        # self.pwm_x = int(1500 + self.pid_controller.control_x/0.523599 * 500)
        # self.pwm_y = int(round(1500 + self.pid_controller.control_y/0.523599 * 500, 0))
        # rc_channel_values[1] =  self.pwm_y
        # rc_channel_values[2] = self.pwm_x
...
class data_base:
    def __init__(self, pid_controller, mav_work):
        self.mavwork = mav_work
    ...
    def append_values(self, elapsed_time, position, error_x, error_y, control_x, control_y):
        """Append current values to all data lists - using passed parameters"""
        ...
        # Command positions (body frame) - these aren't used in your current implementation
        ...
        # Body frame positions - not currently calculated
        # self.x_data_body.append(0.0)  # Placeholder
        # self.y_data_body.append(0.0)  # Placeholder
        
        # Error values (passed as parameters)
        ...
        # Control commands in radians and degrees (body frame)
        self.theta_cmd_radians.append(self.pid_controller.theta_body)
        self.phi_cmd_radians.append(self.pid_controller.phi_body)
        self.theta_cmd_degree.append(math.degrees(self.pid_controller.theta_body))
        self.phi_cmd_degree.append(math.degrees(self.pid_controller.phi_body))
        
        # PWM values
        self.pwm_theta.append(self.pid_controller.pwm_x)
        self.pwm_phi.append(self.pid_controller.pwm_y)
        self.control_x_cmd.append(control_x)
        self.control_y_cmd.append(control_y)
        
        # Rotation data (from QTM tracker)
        if hasattr(self.pid_controller, 'rotation') and self.pid_controller.rotation is not None:
            # Measured values in radians
            self.roll_radians.append(math.radians(self.pid_controller.rotation[0]))
            self.pitch_radians.append(math.radians(self.pid_controller.rotation[1]))
            self.yaw_radians.append(math.radians(self.pid_controller.rotation[2]))
            # Measured values in degrees
            ...
            # Default values if rotation data is not available
            self.roll_radians.append(0.0)
            self.pitch_radians.append(0.0)
            self.yaw_radians.append(0.0)
            self.roll_degree.append(0.0)
            self.pitch_degree.append(0.0)
            self.yaw_degree.append(0.0)

    def save_data(self):
        print("Saving data to CSV file...")
        filename = f'trajectory_data_current.csv'
        try:
            with open(filename, 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                # Define column headers - FIXED: Added missing comma
                writer.writerow([
                    'Time',
                    'x_cmd', 'y_cmd',
                    # 'x_cmd_body', 'y_cmd_body',x_cmd_bod
                    'x_data', 'y_data',
                    ...
                    'x_integral', 'y_integral',  # FIXED: Added missing comma
                    'x_error', 'y_error',
                    'control_x', 'control_y'
                ])
                ...
                rows = zip(
                    ...
                    self.control_x_cmd, self.control_y_cmd 
                )
...
async def main():
    ...
    database = data_base(pid_ctrl, mav_handler)
    ...
    try:
        ...
        # Step 8: Wait for control loop to fiappend_valuesnish
        await control_task
